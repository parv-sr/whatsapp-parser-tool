...
async def _call_llm_api(prompt_user: str, system_prompt: str) -> List[dict]:
...
def _map_results_for_messages(results_list: List[dict], message_count: int, start_global_idx: int) -> List[BatchItemResult]:
    final_results = []
    results_map = {r.get('message_index'): r for r in results_list if isinstance(r, dict)}
    for local_i in range(message_count):
        global_i = start_global_idx + local_i
        if local_i in results_map:
            item_data = dict(results_map[local_i])
...
# --- ASYNC WORKER: Process one "Packet" of messages ---
async def _process_packet(messages: List[str], start_global_idx: int, semaphore: asyncio.Semaphore) -> List[BatchItemResult]:
    async with semaphore:
        ...
        try:
            results_list = await _call_llm_api(prompt_user, system_prompt)
            return _map_results_for_messages(results_list, len(messages), start_global_idx)
        except Exception as batch_err:
            log.warning(
                "Batch packet failed (Indices %s-%s). Falling back to sequential mode: %s",
                start_global_idx,
                start_global_idx + len(messages) - 1,
                batch_err,
            )

            sequential_results: List[BatchItemResult] = []
            for local_i, message in enumerate(messages):
                global_i = start_global_idx + local_i
                try:
                    single_prompt = construct_batch_prompt([message])
                    single_results = await _call_llm_api(single_prompt, system_prompt)
                    mapped_single = _map_results_for_messages(single_results, 1, global_i)
                    sequential_results.extend(mapped_single)
                except Exception as single_err:
                    log.warning("Sequential fallback failed for msg %s: %s", global_i, single_err)
                    sequential_results.append(BatchItemResult(message_index=global_i, listings=[], is_irrelevant=True))
