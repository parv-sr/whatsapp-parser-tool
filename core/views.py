...
def _build_rag_prompt(query: str, contexts: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    system = {
        "role": "system",
        "content": (
            "You are a concise assistant that answers user queries using only the provided context snippets. "
            "If the answer is not present in the snippets, say you couldn't find a definitive answer. "
            "Always cite snippet numbers when referencing facts, e.g., (Snippet 1)."
        )
    }
    ...
    for i, c in enumerate(contexts, start=1):
        ...
        # include short metadata summary if present
        meta_summary = ""
        if isinstance(meta, dict) and meta:
            # Keep it compact
            keys = ["location", "price", "bhk", "building_name"]
            parts = []
            for k in keys:
                if k in meta and meta[k]:
                    parts.append(f"{k}:{meta[k]}")
            if parts:
                meta_summary = " | " + ", ".join(parts)
        context_lines.append(f"[Snippet {i}] {c.get('text', '')}{meta_summary}")

    context_content = "Here are the top relevant snippets from the database (most relevant first):\n\n"
    context_content += "\n\n".join(context_lines)
...
def chat_query(request: HttpRequest):
    ...
    for r in nearest:
        ...
        contexts.append({
            "id": l.id,
            "text": (l.text[:2000] if l.text else ""),  # cap snippet length
